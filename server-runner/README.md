...
`restore-apply --scope=database` can import the staged `database.sql` into the configured staging target after the exact `--confirm=restore-staging-site` phrase is provided. `restore-apply --scope=files` can replace the configured staging target files from staged `htdocs/` after the same confirmation phrase. Both commands first run the existing dry-run/evidence checks for their scope and refuse to apply if any check fails. File apply also reports pre-restore symlinked drop-ins that are absent from the staged files so operators can manually inspect or regenerate them after apply. The runner still refuses `files-and-database` scope in this release. A successful or attempted apply writes a `RESTORE_APPLY_REPORT-*.json` file under `restore_reports_path`.
