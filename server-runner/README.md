...
php alynt-backup-runner.php restore-apply --config=/path/to/config.json --staged-path=/path/to/restores/package-id --scope=files-and-database --pre-restore-evidence=/path/to/pre-restore/evidence.json --confirm=restore-staging-site --format=json
```
