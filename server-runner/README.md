...
`restore-apply --scope=database` can import the staged `database.sql` into the configured staging target after the exact `--confirm=restore-staging-site` phrase is provided. The command first runs the existing dry-run/evidence checks for `database` scope and refuses to import if any check fails. It also refuses `files` and `files-and-database` scopes in this release. A successful or attempted database apply writes a `RESTORE_APPLY_REPORT-*.json` file under `restore_reports_path`.
