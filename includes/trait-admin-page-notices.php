...
trait Alynt_Drime_Backups_Uploader_Admin_Page_Notices {
	...
	private function render_notice( $notice ) {
		$messages = array(
			...
			'dashboard_connection_saved'          => array( 'success', __( 'Dashboard pairing state saved.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_connection_paired'         => array( 'success', __( 'Dashboard pairing completed. The authenticated read-only status endpoint is enabled for dashboard polling.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_connection_pairing_failed' => array( 'error', __( 'Dashboard pairing could not be completed. Confirm the token is unexpired, the client origin matches this site, and the dashboard is reachable.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_connection_invalid_token'  => array( 'error', __( 'The dashboard pairing token could not be accepted. Confirm that it is an unexpired adb1 token from the intended dashboard.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_remote_actions_enabled'    => array( 'success', __( 'V2 action opt-in completed. The site now advertises scan/upload-now capability to the paired dashboard, but no action endpoint is exposed yet.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_remote_actions_failed'     => array( 'error', __( 'V2 action opt-in could not be completed. Confirm the adb2a token is unexpired, belongs to this paired site, and PHP Sodium support is available.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_schedule_apply_enabled'    => array( 'success', __( 'Schedule Apply enabled. The paired dashboard may now request guarded future cadence changes for the Alynt scan/upload schedule after a fresh matching preview.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_schedule_apply_disabled'   => array( 'success', __( 'Schedule Apply disabled. The paired dashboard may still preview the Alynt scan/upload schedule, but it cannot apply cadence changes.', 'alynt-drime-backups-uploader' ) ),
			'dashboard_schedule_apply_failed'     => array( 'error', __( 'Schedule Apply could not be enabled. Confirm read-only pairing, V2 action opt-in, and the local Schedule Apply confirmation checkbox, then try again.', 'alynt-drime-backups-uploader' ) ),
