...
trait Alynt_Drime_Backups_Uploader_Backup_Registry_Failed_Context {
	...
	private function sanitize_failed_context( array $context ) {
		...
		foreach ( array( 'signature', 'name', 'path', 'producer_key', 'producer_label', 'package_id', 'filename', 'backup_set_id', 'manifest_path', 'checksum_path', 'remote_index_path', 'remote_catalog_path', 'checksum_algorithm', 'checksum_value', 'error_code', 'endpoint', 'sidecar_type', 'sidecar_name', 'error_detail' ) as $key ) {
			if ( isset( $context[ $key ] ) && is_scalar( $context[ $key ] ) ) {
				$sanitized[ $key ] = sanitize_text_field( (string) $context[ $key ] );
			}
		}

		foreach ( array( 'size', 'attempts', 'backup_set_index', 'backup_set_total', 'error_status' ) as $key ) {
