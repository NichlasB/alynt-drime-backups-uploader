...
| `alynt_drime_backups_dashboard_connection` | array | disabled connection state | `Alynt_Drime_Backups_Uploader_Dashboard_Connection` | Local central-dashboard pairing intent and future credential metadata. The initial shell keeps the external status endpoint disabled and never stores raw pairing tokens. |
