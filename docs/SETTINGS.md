...
| `alynt_drime_backups_dashboard_connection` | array | disabled connection state | `Alynt_Drime_Backups_Uploader_Dashboard_Connection` | Local central-dashboard pairing intent, reviewed token metadata, dashboard-origin confirmation/revocation state, verifier-only polling credential metadata, V2 action opt-in public-key metadata, and disabled-by-default schedule-apply policy state. The shell keeps the external status endpoint disabled until V1 pairing succeeds, keeps V2 action intents disabled until separate `adb2a` opt-in succeeds, keeps schedule apply disabled until a separate local opt-in is saved, and never stores raw pairing/action tokens or one-time secrets. |
