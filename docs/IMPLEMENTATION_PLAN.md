...
3. Production-capable restore automation. Treat this as a new high-risk gated extension with production-specific rollback, maintenance, ownership, runtime verification, and approval design; staging-only version 1 is complete. The separate living plan is `docs/PRODUCTION_RESTORE_AUTOMATION_PLAN.md`; Phase 0 assumptions are approved and Phase 1 read-only GridPane investigation is complete.
