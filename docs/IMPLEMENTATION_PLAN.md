...
- Current post-release proof gate: `v0.5.21` is released and deployed across the tracked dashboard/client rollout set. Runtime rollback-preview proof still requires a separately approved pilot with fresh, unexpired rollback metadata from a recent successful `schedule_apply`; existing older apply metadata is not reusable because the client expires rollback metadata after one hour. A first approved PureCleanse pilot stopped before any cadence mutation because the client accepted a non-mutating preview, then exposed that schedule-management previews were paced by the generic one-hour action interval even though previews expire after 15 minutes. The follow-up patch keeps scan/upload throttling conservative while giving schedule-management actions a shorter local retry window. `schedule_rollback` remains unavailable.
...
- Automatic pre-restore backup creation passed a real combined staging restore rehearsal on `alyntdrime.sitesmain.com` with package `alyntdrime-sitesmain-com-20260712-173228`.
...
Status: released in uploader `v0.5.19`. Client-side Schedule Apply remains disabled by default and requires separate local opt-in before capability reporting advertises apply support.
...
- Keep rollback execution, WPvivid schedule changes, server-runner schedule changes, backup creation, cleanup/delete, restore, arbitrary commands, and Drime credential changes out of scope.
- Treat rollback metadata capture/readiness as the next separate planning slice; it must not enable `schedule_rollback`.
...
- Apply result reports rollback unavailable; dedicated rollback metadata capture/readiness remains a future slice.
...
Remaining after release/deploy:

- Keep client-side Schedule Apply disabled unless a site is explicitly approved for local opt-in.
- Monitor live/pilot evidence for schedule-apply stability.
- Plan rollback metadata capture/readiness as a separate slice before any `schedule_rollback` action is designed.
- Keep `schedule_rollback` unavailable until separately approved, implemented, reviewed, released, and proven.

### Dashboard V2.3 Rollback Metadata Capture / Readiness Slice

Status: released in uploader `v0.5.20`. No runtime rollback behavior is approved.

Goal:

- Capture bounded support-safe previous-schedule metadata after a successful `schedule_apply` so future rollback design has reliable evidence.
- Keep `schedule_rollback` rejected/unavailable.
- Avoid exposing raw cron arrays, option blobs, filesystem paths, shell/user context, backup package names, Drime IDs, credentials, or arbitrary settings.

Planning/scope artifact:

- Dashboard `docs/V2_3_ROLLBACK_METADATA_CAPTURE_PLAN.md`
...
- Let the dashboard request a non-mutating rollback-readiness preview for one previous successful `schedule_apply` record.
- Validate that the source apply record exists, succeeded, still has matching rollback metadata, and that the current local Alynt scan/upload schedule still matches the post-apply fingerprint.
- Return only support-safe scalar evidence for operator review.
- Keep actual rollback execution, schedule rollback apply, WPvivid schedule changes, server-runner schedule changes, backup creation, restore, delete, cleanup, arbitrary settings, filesystem paths, commands, and Drime credential actions unavailable.

Implemented local direction:

- Added a separate local rollback-preview opt-in after V2 action opt-in; default disabled.
- Accepted only the bounded `schedule_rollback_preview` action with `schedule_id`, `source_apply_action_id`, `rollback_metadata_fingerprint`, and `capability_version`.
- Validated the source `schedule_apply` action, rollback metadata fingerprint, metadata expiry, allowlisted cadence labels, and current schedule fingerprint before returning readiness evidence.
- Reported `rollback_preview_supported` only when the client is paired, V2 actions are opted in, the Alynt scan/upload schedule is manageable, and the separate local rollback-preview policy is enabled.
- Stored sanitized rollback-preview results in the existing bounded remote-action state/audit store, forcing `rollback_supported` and rollback apply support to false.

Boundaries:

- `schedule_rollback_preview` is absent unless local rollback-preview policy is explicitly enabled.
- The preview targets only `alynt_scan_upload`.
- The preview does not call WordPress schedule mutation APIs.
- `schedule_rollback` remains unsupported and is not dispatchable.
- Rollback preview support does not grant rollback execution.
- Client rollout, release packaging, dashboard pilot proof, and any future rollback execution design remain separate gates.

Validation on 2026-09-22:

- Feature Light Review: passed for the local rollback-preview/supporting refactor surface; the slice touches REST action parsing, option state, status capability reporting, bounded action storage, WP-Cron schedule inspection, and tests.
- Feature Bloat And Structure Review Phase 1: explicit base ref `fe0adda`; changed production PHP files are all below the 300-line feature threshold. Oversized changed files are test classes only (`tests/RemoteActionIntentEndpointTest.php`, `tests/RemoteActionWorkerTest.php`, `tests/DashboardConnectionTest.php`) and are deferred to a later test-structure pass rather than changed in this behavior slice.
...
- Real automatic pre-restore backup creation rehearsal passed on GridPane staging `alyntdrime.sitesmain.com` on 2026-07-12: runner source commit `7445594` was installed, package `alyntdrime-sitesmain-com-20260712-173228.tar.gz` was created, verified, and staged; `restore-apply --scope=files-and-database --create-pre-restore-backup=1 --confirm=restore-staging-site` created pre-restore database/file evidence, replaced files, imported the database, and wrote `RESTORE_APPLY_REPORT-alyntdrime-sitesmain-com-20260712-173228-20260712-173925.json`; post-apply WP-CLI URL, core, database, and HTTPS `200` checks passed; large package, staging directory, pre-restore file tarball, and temporary restore config were removed; small evidence/report files plus the pre-restore database export were retained.
