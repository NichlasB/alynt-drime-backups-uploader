...
3. Fetch the package and sidecars from Drime, or download all three manually to a safe server path.
...
- Fetch or manual download includes all three artifacts.
- Verification passes.
...
| Restore fetch cannot find sidecars | Confirm all three Drime artifacts share the same package basename and are in the same destination folder. |
