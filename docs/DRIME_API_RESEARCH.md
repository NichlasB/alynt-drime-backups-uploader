...
- Multipart chunk size is configurable from 5 MB to 256 MB and defaults to 128 MB. The 128 MB recommendation is intended for large backups on hosts with enough PHP memory and stable network behavior. The uploader checks PHP memory before reading a part so oversized settings fail gracefully instead of exhausting memory.
...
- The plugin now stores resolved relative-path parent folder IDs in `alynt_drime_backups_drime_locations`.
...
- Live abortable part-size probing found Drime accepted signed multipart `PUT` uploads with 32 MB and 64 MB parts and returned ETags for both. Later server-backup testing shifted the production-oriented recommendation to 128 MB for large packages when PHP memory supports one part plus runtime overhead. Larger 192 MB and 256 MB settings are allowed for validation but still need fresh LocalWP/Drime E2E evidence before being recommended broadly.
